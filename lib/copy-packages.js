...
function getPackageDependencies(basedir) {
	...
	// TODO: las package dependencies no se cogen a partir del directorio del paquete sino de su padre (porque pueden ser hermanas :-o)
	...
	var dependencyDirs = Object.keys(packageJson.dependencies).map(function(
		dependency
	) {
	...
	console.log('Package ' + pkgId + ' -> ' + Object.keys(pkgs).join(', '));
...
function resolveDependencyDir(packageDir, dependency) {
	if (resolveModule.isCore(dependency)) {
		return null;
	}
...
function copyPackages(options) {
	...
	var streams = Object.keys(pkgs).map(function(key) {
		...
		if (debug) {
			console.log('Copying package:', pkg.dir.substring(process.cwd().length));
		}
