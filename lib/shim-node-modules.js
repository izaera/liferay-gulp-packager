...
function getReplacements(file, nodeModules) {
	try {
		var replacements = [];
...
function gulpShimNodeModules(debug, nodeModules, pkgJsonPatches) {
	return util.transform(function(file) {
		var replacements = getReplacements(file, nodeModules);
		...
		if (replacements.length > 0) {
			...
			// Store replacements in output pkgJsonPatches parameter
			...
			replacements.forEach(function(replacement) {
				...
				var moduleName = replacement.to;

				moduleName = moduleName.substring(1, moduleName.length - 1);

				patches[moduleName] = '1.0.0';
			});
		}
	});
}
